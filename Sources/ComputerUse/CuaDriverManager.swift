...
final class CuaDriverManager {
    ...
    func start(
        ...
        process.terminationHandler = { [weak self, weak runningSession] process in
            ...
            Task { @MainActor in
                self?.handleTermination(session: runningSession, status: status)
            }
        }
        ...
        do {
            ...
            if self.session === runningSession {
                runningSession.isStopping = true
                terminate(process)
                session = nil
    ...
    func stop() async {
        ...
        do {
            try await withTimeout(.seconds(3)) {
                try await session.terminationInbox.next()
            }
        } catch {
            if process.isRunning {
                kill(process.processIdentifier, SIGKILL)
            }
            _ = try? await session.terminationInbox.next()
        }
    ...
    private func handleTermination(session terminatedSession: CuaDriverProcessSession?, status: Int32) {
...
private final class CuaDriverProcessSession {
    let process: Process
    let stdin: Pipe
...
private actor CuaDriverLineInbox {
