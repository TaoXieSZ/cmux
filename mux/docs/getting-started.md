...
The usual default is `${TMPDIR:-/tmp}/cmux-mux-$(id -u)/main.sock`. `--session <name>` changes the final file name. `--socket <path>` bypasses the session-derived path. Server-started child processes receive `CMUX_MUX_SOCKET` with the socket path.

## Platforms and XDG

The mux socket uses Rust's temporary directory on every platform. The TUI config path is `~/.config/cmux/mux.json` unless `CMUX_MUX_CONFIG` is set.
