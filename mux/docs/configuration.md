...
`cmux-mux` reads `~/.config/cmux/mux.json`. Set `CMUX_MUX_CONFIG` to use another file. Every documented key is optional. Unknown keys in the typed sections make the raw config invalid, so the TUI logs an error and falls back to defaults.
